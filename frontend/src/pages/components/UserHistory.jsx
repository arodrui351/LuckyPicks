...
export default function UserHistory() {
    const { id } = useParams(); // Capturar el ID del usuario desde la URL
                    ...
                    <table>
                        ...
                        <tbody>
                            {sessionsCount.length ? sessionsCount.map(s => (
                                <tr key={s.name}><td>{s.name}</td><td>{s.plays}</td></tr>
                    ...
                    <table>
                        ...
                        <tbody>
                            {transactions.length ? transactions.map(tx => (
                                <tr key={tx.id}><td>{tx.amount}</td><td>{new Date(tx.created_at).toLocaleString()}</td></tr>
                    ...
                    <table>
                        ...
                        <tbody>
                            {sessionsDetail.length ? sessionsDetail.map(s => (
                                <tr key={s.id}><td>{s.game_name}</td><td>{s.bet_amount}</td><td>{s.win_amount}</td><td>{new Date(s.started_at).toLocaleString()}</td></tr>
